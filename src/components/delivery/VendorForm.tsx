...
import adminUsers, { CreateUserPayload, UpdateUserPayload, User } from "@/app/api/usersmanagement";
...
export default function DeliveryForm() {
  ...
  useEffect(() => {
    if (isEdit && id) {
      const fetchUser = async () => {
        try {
          const user: User = await adminUsers.getUserById(id);
          setFormData({
          ...
          toast.error(err.body?.message || "Failed to load deliveryman");
        }
      };
      fetchUser();
    }
  ...
  const validatePhone = (phone: string): string => {
    ...
    return phoneRegex.test(phone) ? "" : "Please enter a valid phone number (e.g., 0549461543).";
  };
  ...
  const validateName = (name: string): string => {
    ...
    return name.length > 0 && nameRegex.test(name) ? "" : "Please enter a valid name (letters only).";
  };
  ...
  const validateForm = () => {
    const newErrors = {
      ...
      confirmPassword: isEdit ? "" : (formData.password !== formData.confirmPassword ? "Passwords do not match." : ""),
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const t = toast.loading(`${isEdit ? "Updating" : "Adding"} deliveryman...`);
    ...
    try {
      ...
      toast.dismiss(t);
      toast.success(`${isEdit ? "Deliveryman updated" : "Deliveryman added"} successfully`);
      router.push("/dashboard/deliverymen");
    } catch (err: any) {
      toast.dismiss(t);
      const msg = err.body?.message || "Failed to save deliveryman";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-[1247px] mx-auto px-8 py-6">
        <div className="flex items-center gap-4 mb-12">
          ...
          <h1 className="text-[#292D32] text-2xl font-medium tracking-[0.505px]">
            {isEdit ? "Edit Deliveryman" : "Add Deliveryman"}
          </h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-[#F1F1F1] rounded-xl p-12">
          <div className="max-w-[895px]">
            ...
            >
              {loading ? `${isEdit ? "Updating" : "Adding"}...` : (isEdit ? "Update deliveryman" : "Add deliveryman")}
            </button>
          </div>
        </form>
      </div>
