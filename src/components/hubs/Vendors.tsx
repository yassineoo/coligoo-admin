...
export default function Hubs() {
  ...
  const fetchHubs = async () => {
    setLoading(true);
    setError(null);
    const t = toast.loading("Loading hubs...");
    try {
      const params = {
        page: currentPage,
        pageSize,
      };
      const res = await adminUsers.getHubAdminUsers(params);
      ...
      toast.dismiss(t);
      toast.success("Hubs loaded successfully");
    } catch (err: any) {
      const msg = err.body?.message || "Failed to load hubs";
      toast.dismiss(t);
      toast.error(msg);
      setError(msg);
      setVendors([]);
  ...
  const handleBulkDelete = async () => {
    ...
    if (selected.length === 0) {
      toast.warning("No hubs selected");
      return;
    }
    if (!confirm(`Delete ${selected.length} hubs?`)) return;
    ...
    try {
      ...
      toast.dismiss(t);
      toast.success(`Deleted ${selected.length} hubs`);
      fetchHubs();
    } catch (err: any) {
      toast.dismiss(t);
  ...
  const handleBulkDeactivate = async () => {
    ...
    if (!confirm(`Deactivate ${selected.length} hubs?`)) return;
    ...
    try {
      ...
      toast.dismiss(t);
      toast.success(`Deactivated ${selected.length} hubs`);
      fetchHubs();
    } catch (err: any) {
      toast.dismiss(t);
  ...
  const handleBulkActivate = async () => {
    ...
    if (!confirm(`Activate ${selected.length} hubs?`)) return;
    ...
    try {
      ...
      toast.dismiss(t);
      toast.success(`Activated ${selected.length} hubs`);
      fetchHubs();
    } catch (err: any) {
      toast.dismiss(t);
      ...
      <div className="max-w-[1262px] mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <ActionButton
              ...
              onClick={() => router.push("/dashboard/hubs/create")}
            >
              Add Hub
            </ActionButton>
          </div>
        ...
        {error ? (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-red-700">
            {error}
            <button onClick={fetchHubs} className="ml-2 underline">
              Retry
            </button>
          </div>
        ) : hasData ? (
          ...
          <div className="bg-white rounded-lg shadow border border-gray-100 h-[420px] flex items-center justify-center">
            <div className="text-center max-w-md">
              ...
              />
              <div className="text-lg text-gray-900">No hubs for now</div>
