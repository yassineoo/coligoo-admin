...
export default function LockersComponent() {
  ...
  const handleRowClick = (locker: Locker) => {
  ...
  const handleDeleteClick = (lockerId: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleDeleteConfirm = () => {
    if (lockerToDelete) {
      setLockers((prev) => prev.filter((l) => l.id !== lockerToDelete));
      setSelectedLockers((prev) => {
        const next = new Set(prev);
        next.delete(lockerToDelete);
        return next;
      });
  ...
  const handleDeleteMultipleConfirm = () => {
    setLockers((prev) => prev.filter((l) => !selectedLockers.has(l.id)));
    setSelectedLockers(new Set());
  };
        ...
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex items-center justify-end gap-4">
            <button
              ...
              disabled={selectedLockers.size === 0}
              className="inline-flex items-center px-3 py-2 text-sm font-medium text-[#292D32] bg-white rounded-lg hover:bg-gray-50 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Trash2 className="w-4 h-4 mr-2" />
              Delete
            </button>
            <button
              onClick={() => router.push("/dashboard/lockers/create")}
            ...
            <div className="min-w-max">
              ...
              {currentLockers.map((locker) => (
                <div
                  ...
                  onClick={() => router.push(`/dashboard/lockers/${locker.id}/edit`)}
                >
                  <div
                  ...
                  <div className="px-2 flex justify-center">
                    <div className="relative inline-flex items-center">
                      <select className="appearance-none bg-[#DFFFEA] text-[#0B8457] text-sm font-medium px-3 py-1 pr-8 rounded-lg cursor-pointer">
                  ...
                  >
                    <button
                      ...
                      className="w-8 h-8 rounded-lg bg-[#FFEBE5] flex items-center justify-center hover:bg-[#FFD5CA] transition"
                    >
                      <img src="/icons/trash.svg" alt="Delete" />
                    </button>
                    <button
                      onClick={() => handleRowClick(locker)}
                      className="w-8 h-8 rounded-lg bg-[#E6F4FF] flex items-center justify-center hover:bg-[#CCE8FF] transition"
                    >
                      <img src="/icons/info-circle.svg" alt="Edit" />
                    </button>
                  </div>
                </div>
              ))}
      ...
      <DeleteConfirmationModal
        ...
        title={
          lockerToDelete
            ...
            : `Are you sure you want to delete ${selectedLockers.size} locker(s)?`
        }
