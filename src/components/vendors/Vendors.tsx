...
export default function Vendors() {
  ...
  const fetchVendors = async () => {
    setLoading(true);
    setError(null);
    const t = toast.loading("Loading vendors...");
    try {
      const params = {
        page: currentPage,
        pageSize,
      };
      const res = await adminUsers.getVendorUsers(params);
      ...
      const msg = err.body?.message || "Failed to load vendors";
      toast.dismiss(t);
      toast.error(msg);
      setError(msg);
      setVendors([]);
  ...
  const handleBulkDelete = async () => {
    ...
    if (selected.length === 0) {
      toast.warning("No vendors selected");
      return;
    }
    ...
    try {
      ...
      toast.dismiss(t);
      toast.success(`Deleted ${selected.length} vendors`);
      fetchVendors();
    } catch (err: any) {
      toast.dismiss(t);
  ...
  const handleBulkDeactivate = async () => {
    ...
    try {
      ...
      toast.dismiss(t);
      toast.success(`Deactivated ${selected.length} vendors`);
      fetchVendors();
    } catch (err: any) {
      toast.dismiss(t);
  ...
  const handleBulkActivate = async () => {
    ...
    if (!confirm(`Activate ${selected.length} vendors?`)) return;
    ...
    try {
      ...
      toast.dismiss(t);
      toast.success(`Activated ${selected.length} vendors`);
      fetchVendors();
    } catch (err: any) {
      toast.dismiss(t);
      ...
      <div className="max-w-[1262px] mx-auto">
        ...
        {error ? (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-red-700">
            {error}
            <button onClick={fetchVendors} className="ml-2 underline">
              Retry
            </button>
          </div>
        ) : hasData ? (
          ...
          <div className="bg-white rounded-lg shadow border border-gray-100 h-[420px] flex items-center justify-center">
            <div className="text-center max-w-md">
              ...
              />
              <div className="text-lg text-gray-900">No vendors for now</div>
